...
class NlgModel:
    def __init__(self, model_path, tokenizer_path, device=None):
        ...
        state_dict = torch.load(f"{save_directory}/pytorch_model.bin", map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        self.model.load_state_dict(state_dict)  # Load the fine-tuned weights
...
# Specify the path to the saved model directory in Google Drive
...
nlgModel.respond_to_input("Find me a 10 km trail near my current location.")
